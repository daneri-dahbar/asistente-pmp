#!/usr/bin/env python3
"""
Script mejorado para generar datos de demostración del Asistente PMP
Crea un usuario demo con múltiples sesiones y mensajes distribuidos
equilibradamente entre todos los modos para mostrar el potencial completo.
"""
...
def create_balanced_demo_sessions(user_id):
    """Crear sesiones de demostración con distribución equilibrada entre modos"""
    db = SessionLocal()
    ...
    sessions_created = 0
    mode_counts = {mode: 0 for mode in STUDY_MODES}
    
    # Distribución objetivo por modo (más sesiones para modos analíticos)
    target_sessions = {
        "CHARLEMOS": 20,    # Chat libre
        "ESTUDIEMOS": 25,   # Estudio estructurado
        "EVALUEMOS": 30,    # Evaluaciones (importante para analytics)
        "SIMULEMOS": 30,    # Simulaciones (importante para analytics)
        "ANALICEMOS": 15    # Análisis de progreso
    }
    
    # Crear sesiones por modo para asegurar distribución equilibrada
    for mode in STUDY_MODES:
        target_count = target_sessions[mode]
        
        for session_num in range(target_count):
            # Seleccionar fecha aleatoria en los últimos 30 días
            ...
            # Seleccionar tema apropiado para el modo
            ...
            # Crear mensajes para esta sesión (más mensajes para modos analíticos)
            if mode in ["EVALUEMOS", "SIMULEMOS"]:
                num_messages = random.randint(8, 15)  # Más interacción
            elif mode == "ANALICEMOS":
                num_messages = random.randint(5, 10)  # Análisis más conciso
            else:
                num_messages = random.randint(4, 12)  # Conversación normal
    ...
    print("📊 Distribución por modo:")
    for mode, count in mode_counts.items():
        print(f"   {mode}: {count} sesiones")
...
def generate_demo_data():
    ...
    print("🚀 Generando datos de demostración MEJORADOS para Asistente PMP...")
    print("=" * 60)
    ...
    # Crear sesiones y mensajes con distribución equilibrada
    sessions_count = create_balanced_demo_sessions(demo_user.id)
    ...
    mode_stats = {}
    for mode in STUDY_MODES:
        mode_sessions = db.query(ChatSession).filter(
            ChatSession.user_id == demo_user.id,
            ChatSession.mode == mode
        ).count()
        mode_stats[mode] = mode_sessions
    
    print("=" * 60)
    print("📊 RESUMEN DE DATOS DE DEMOSTRACIÓN MEJORADOS:")
    ...
    for mode, count in mode_stats.items():
        print(f"      {mode}: {count} sesiones")
    print("=" * 60)
    print("✅ Datos de demostración mejorados generados exitosamente!")
    print("   Ahora puedes iniciar sesión con 'demo_user' y 'demo123'")
    print("   para ver el potencial completo de la aplicación.")
    print("   Especialmente en el modo ANALICEMOS verás datos ricos!")
